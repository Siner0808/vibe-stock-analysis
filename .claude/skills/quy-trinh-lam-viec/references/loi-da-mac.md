...
| 14 | **sáu cửa chưa bao giờ chạy** — phiên luôn mở ngoài repo | phép thử ở phiên sau | ⚠️ một phần | `~/.claude/rules/vibe-preview.md` |
...
| 34 | gác mới chỉ chạy trên dữ liệu SẠCH, nên **mọi đột biến NỚI LỎNG nó đều sống sót** — nới một phép kiểm ra thì nó vẫn xanh trên đầu vào sạch | đục thử: 3/6 sống, cả ba đều là phép nới | ✅ | `tests/test_bang_loi_do_duoc.py` — tách phép phán thành hàm thuần rồi thử bằng CẢ đầu vào phải-qua LẪN phải-chặn |

| 35 | dòng tự khai ở cuối bảng này được **cộng dồn** mỗi lần thêm một lỗi thay vì **đếm lại** — lệch +1 suốt ba ngày, qua hơn mười commit | `tools/doc_bang_loi.py` ngay lượt chạy đầu tiên | ✅ | `tools/doc_bang_loi.py`: nó đọc số viết bằng chữ tiếng Việt rồi đối chiếu với số đếm được, mã thoát 1 khi lệch |
...
| 42 | bỏ qua một **chỉ dẫn thường trực** ghi ở BA nơi (`SKILL.md`, rules toàn cục, bộ nhớ phiên) suốt bốn phép đo liên tiếp — người dùng phải nhắc **hai lần** | người dùng | ✅ | `docs/soat-notebooklm.json` + `tests/test_soat_notebooklm.py` (gác của dòng 41): một chỉ dẫn không có cơ chế thì nó chỉ là một lời nhắc, và lời nhắc thì trôi |
...
| 51 | **rà soát cả 8 luật: 13/14 hình dạng LỌT.** Ba luật khai hẹp hơn cơ chế chúng nêu — `sed -i` bỏ sót dạng dài `--in-place`, `python-he-thong` bỏ sót `python3.11` và `py`, `pytest-qua-ong` bỏ sót mọi lệnh có `2>&1`. Lỗi 48 trông như vấn đề của **một** luật; đếm cả tám thì nó là vấn đề của **hệ** | đo, sau khi lỗi 48 để ngỏ đúng câu hỏi này | ⚠️ **4/13** | nới ba luật, mỗi phép nới đo bắt nhầm trước trên 20 lệnh `TOT` + 69 dòng lệnh tài liệu, cả ba ra **0**. Còn **9 hình dạng chưa đóng**, kể tên trong `docs/STATE.md` BƯỚC 65. **NHÓM 1 ĐÃ ĐÓNG 17/09/2026** bằng ba luật MỚI — `ghi-de-file-nguon` · `ghi-de-db` · `va-tai-cho-khac-sed`; đo trước khi bật trên 23 mẫu `TOT` + 73 dòng lệnh tài liệu, **0 bắt nhầm**. Còn để ngỏ: `cp`/`mv` có đích là file nguồn (biểu thức chuỗi không biết đích có tồn tại không), `git push` trần trên `main` (trạng thái LÚC CHẠY), và `mv x.db /tmp` (**cố ý** không chặn). `docs/STATE.md` BƯỚC 92 |
...
| 56 | **để escape xuyên qua heredoc làm hỏng file — BỐN LẦN trong một ngày**. `\n` viết trong thân heredoc thành xuống dòng thật, cắt đôi một chuỗi Python. Skill cấm điều này bằng một câu (*"vá lớn thì viết một file `.py` rồi chạy, không bao giờ qua escape lồng trong heredoc"*) và tôi vẫn quay lại đường ấy mỗi khi phép vá **trông nhỏ** | chính Python: `SyntaxError: unterminated f-string` | ✅ | **ĐÃ DỰNG 17/09/2026, sau lần thứ NĂM.** Không dựng lại ứng viên đã rút — dựng một luật KHÁC, đứng trên một cơ chế **đo được**: một dấu chéo ĐƠN sống sót đúng qua heredoc (`'a\nb'` → len 3), hai dấu chéo thì **bị thu về một** (`'a\\nb'` → len 3, đáng lẽ 4). Nên luật chặn **dấu chéo nhân đôi trong thân một heredoc nạp Python**, không chặn `\n` và không chặn mọi heredoc Python. Luật `heredoc-python-co-escape` của `tools/cua_bash_an_toan.py`; đo trước khi bật: 0 bắt nhầm trên 32 mẫu TỐT và 534 khối lệnh tài liệu. Xem dòng 58 — điều kiện nó đặt ra đã được thoả |
...
| 61 | **năm máy đo trong hai ngày, mỗi cái HẸP HƠN thứ nó đo — và mỗi cái đều in ra một con số nghe hợp lý.** (a) quét câu song đôi so TỪNG DÒNG, mà tài liệu ngắt ở cột ~76 → **0 ca**, thật là 1. (b) quét quần thể heredoc đòi `-` đứng liền `<<`, thực tế có `> log 2>&1` chen giữa → **0 ca**, thật là 5. (c) phép đục thử của gác index không đi qua `bi_che()` nên phát thứ năm **sống sót giả**. (d) quét liên kết test↔bảng đọc *"lỗi N"* mà không tách **sinh ra từ** khỏi **áp bài học** → **16/20 bắt nhầm**. (e) quét địa chỉ dấu ✅ chỉ nhận đường dẫn đầy đủ → **15/33** sai, 12 trong số đó có địa chỉ thật. Cùng hình dạng lỗi 30/35/36/38 và 48, lần này ở **MÁY ĐO** chứ không ở GÁC — và một máy đo sai thì không đỏ, nó chỉ ra một con số | bốn lần do **in dữ liệu thô bên dưới con số**, một lần do **có một ca thật đã biết trước** | ⚠️ | không gác được: một máy đo mới thì chưa có quần thể nào để đo độ mù của chính nó. Thay bằng điều bắt buộc **số 4** ở `SKILL.md` Bước 3 — in dữ liệu thô dưới mọi con số, và bắt máy đo đi qua ít nhất một ca THẬT đã biết trước khi tin nó |

| 62 | **tôi khai một công cụ MẠNH ở đâu mà chưa đo bao giờ — rồi commit lời khai ấy.** `docs/STATE.md` BƯỚC 74 (14/09) viết: *tài liệu mâu thuẫn tài liệu là "sân mạnh nhất" của NotebookLM*, dựa trên đúng **một** ca thành công (lỗi 46). Đo 15/09 trên chính lớp ấy: **0 phát hiện thật / 4 lượt nêu** qua hai phiên — lượt 14/09 (nguồn cũ, không chỉ dẫn) nêu 2 và **2/2 bắt nhầm**, đều là trích dẫn trong khối 🔴; lượt 15/09 (nguồn `main` tươi, kèm chỉ dẫn *bỏ qua khối đánh dấu*) nêu **0** và **bỏ lọt cả hai** mâu thuẫn thật mà tôi tìm bằng tay trong mười phút. Chỉ dẫn của tôi đổi một bài toán bắt-nhầm thành một bài toán bỏ-lọt | chạy đúng lớp ấy hai lần rồi đếm, sau khi đã trích lời khai ấy một ngày | ⚠️ | sổ khai thêm `_gioi_han_QUY_UOC_DANH_DAU` — quy ước *giữ câu cũ kèm dấu* của dự án và cách đọc của công cụ **xung đột về cấu trúc**, nên mọi phép sửa đúng quy ước lại thành một mồi bắt nhầm. Luật: mỗi *mâu thuẫn* nó nêu phải kiểm bằng `grep -n` + đọc bối cảnh trước. **Không rút công cụ** — n vẫn nhỏ (2 mâu thuẫn, 2 cách hỏi) và lỗi 46 là ca ngược |

| 63 | **đưa một lý do THỰC NGHIỆM ở chỗ có sẵn một lý do CẤU TRÚC — và lý do yếu ấy sống 17 ngày.** `docs/STATE.md` BƯỚC 8 (29/08) ghi `tv_recommendation` vi phạm bất biến 2 (`STRONG_BUY`→`NEUTRAL`, điểm B **75 → 67**, tức 8 điểm trên thang có ngưỡng 62) rồi kết: *"hiện vô hại vì bonus không đổi quyết định mã nào"*. Đó là **n=1**: đúng hôm ấy, có thể sai ngày mai — nên nó mời người sau đi đo tính tái lập, một phép đo không thể đổi con số nào. Đọc AST 15/09 cho câu mạnh hơn hẳn: **đường giao dịch không THỂ chạm tới TradingView thật** — `paper_runner._analyze` và `backtest/engine.py` đều ghim cứng `"NEUTRAL"`, và `collect_and_handoff()` chỉ được gọi từ `master_agent.run_full_analysis`. `tv_bonus` bằng 0 cho MỌI lệnh trong sổ | đi tra đường gọi trước khi thiết kế phép đo — đúng gác của lỗi 41/53 | ✅ | `tests/test_duong_giao_dich_khong_doc_tradingview.py`: bốn module sinh lệnh không được gọi `collect_and_handoff()`, và mọi lần truyền `tv_recommendation` phải là HẰNG SỐ. Đục thử 5/5 đỏ, phát đầu dựng lại nguyên văn mối nguy. Nó bắt **0 ca hôm nay** — thứ nó canh là một thay đổi nghe rất hợp lý: *"dùng TradingView thật trong lượt quét đi"* |
...
**Năm mươi chín trên một trăm lẻ ba máy chặn được.** Lỗi 4 hoá ra không phải lỗi
...
| 65 | **cái trần giữ đúng, mà bản báo cáo của chính nó tố nó vượt — ở BA nơi cùng lúc.** `_capital_deployment` cộng `+size`/`-size` theo mốc ngày; mọi `size_pct` là bội số 0,1 điểm, mà 0,1 không biểu diễn chính xác được bằng nhị phân. Đo trên `wf_oos.db` lượt D của ĐO 9 (792 lệnh, đỉnh 2023-05-17): tổng **Decimal đúng 100,0000**, tổng **float 100,0000000000001**, bụi 1e-13 — và `> 100.0` kêu **ĐÒN BẨY ẨN** trong khi `consider_entry` chưa cho vượt một xu. Ba nơi tự viết phép so ấy: `walkforward.py:569`, `app.py:1611`, `paper_metrics.Performance.is_leveraged` — nên cảnh báo đỏ ấy đang hiện **trên app của người dùng**. Nửa thứ hai: cả ba in `:.0f`, nên **99,9 · 100,0 · 100,4 ra cùng chuỗi "100%"** và người đọc không kiểm được phán quyết bằng chính con số bên cạnh | chốt chặn đã ký của ĐO 9 cắn vào **lượt đối chứng** — một cổng mà hệ đang chạy cũng không qua thì cổng ấy đáng ngờ trước | ✅ | `paper_metrics.vuot_tran_von()` + `DUNG_SAI_VON_PCT = 1e-6`, hai biên đều ĐO ĐƯỢC (bụi 1e-13 · bước yết 0,1). `tests/test_tran_von_khong_bao_nham.py` quét AST cả ba file: không nơi nào được so tên vốn-cam-kết với hằng số ≥ trần, và mỗi nơi phải GỌI hàm phán. **9/9 đột biến đỏ lượt đầu** |

| 66 | **kết luận "không có bụi" từ một mẫu không thể có bụi.** Đo lượt C thấy đỉnh **99,9** với bụi 1e-14, tôi viết *"bụi float bị loại, suy ra A và B vượt 100% thật"*. Vô hiệu: 99,9 không bao giờ chạm `> 100.0`, nên phép thử ấy **không thể** cho kết quả dương dù sự thật là gì — một kết luận phủ định rút từ một mẫu không có khả năng phủ định nó. Sống **33 phút** và đi vào một tin nhắn báo cáo. Cùng họ lỗi 61, khác chỗ: ở đó cái hẹp là **công cụ**, ở đây cái hẹp là **lập luận** còn công cụ thì đúng | lượt D chạm ĐÚNG 100 — tức ca thật đã biết trước mà điều bắt buộc số 4 ở `SKILL.md` Bước 3 đòi phải có | ⚠️ | không gác được bằng test: đây là lỗi đọc, không phải lỗi mã. Luật rút ra, thêm vào `SKILL.md` Bước 3: **trước khi tin một kết quả ÂM, hỏi mẫu này có khả năng cho kết quả DƯƠNG không.** Mẫu không nằm trong vùng phép thử phân biệt được thì kết quả âm của nó nói về mẫu, không nói về giả thuyết |
...
| 69 | **một nhãn hứa lợi nhuận KHÔNG GIỚI HẠN trong khi giá đi kèm là đúng +20%.** `analysis_agents.py` đặt `take_profit_pct = "Không giới hạn"` và `tp2_pct = "Vô cực"`, trong khi **năm** nơi tiêu thụ in chúng kèm hậu tố `%` — ra thành `+Không giới hạn%`. Nặng nhất là `debate_agents.py`: nó dựng một luận điểm Risk:Reward đặt rủi ro **đã định lượng** (−4,0%) cạnh lợi nhuận **vô hạn** rồi kết luận *"hoàn toàn có thể chấp nhận"*, đặt trước mặt người sắp quyết định. Sinh **10/08/2026** (`025507c`), sống **37 ngày** — dài nhất bảng này, vượt lỗi 65. Nơi tiêu thụ chờ một con số từ commit ĐẦU TIÊN (03/08); bảy ngày sau nơi sản xuất đổi sang câu chữ | tra `tools/ho_so.py` thấy file này có **0 tham chiếu**, rồi CHẠY đường ấy thay vì đọc mã | ✅ | `tests/test_khuyen_nghi_rui_ro.py` — quét AST cả repo tìm khoá `*_pct` mang chuỗi, cộng một gác đối chiếu phần trăm với giá cùng cặp; đục thử 4/4 đỏ, gồm một phát đục vào chính máy quét |
| 70 | **nhãn nói CHẶT hơn mã.** `analysis_agents.py` mô tả chính sách thoát lệnh là *"Trailing Stop 7% từ ĐỈNH"*, trong khi `paper_trading.py` đặt `trail_sl = close_p * 0.93` — 7% dưới giá **đóng cửa**, không phải dưới `high`. Đóng cửa ≤ đỉnh nên stop thật **lỏng hơn** thứ nhãn hứa. Hai vế sinh ra trong **cùng một commit** `025507c` (10/08/2026): cùng một lượt viết, gõ `close_p` rồi mô tả là *đỉnh*. Sống **37 ngày** | đi tìm xem cái chuỗi sắp bị thay có mang thông tin thật nào không, rồi đọc mã trailing để kiểm | ✅ | `tests/test_khuyen_nghi_rui_ro.py::test_CHINH_SACH_TRAILING_khong_mat_khi_RR_thanh_so` — đòi chính sách phải còn được nói ra ở `signals`, và chặn đúng chữ *đỉnh* quay lại |
| 71 | **ba con số BỊA trong một câu tự nhận là "đã ĐỊNH LƯỢNG".** `debate_agents.py` dùng `rec.get("stop_loss_pct", 7)`, `…("take_profit_pct", 17)`, `…("risk_reward_ratio", "2.5:1")`. Khối khuyến nghị vắng mặt thì Bull vẫn phát ra *"Rủi ro đã được ĐỊNH LƯỢNG và kiểm soát! Stop-loss chỉ −7%, upside +17%, RR 2.5:1"* — ba con số không ai đo, trong đúng một câu quảng cáo sự định lượng của nó. Luật **R4** của `chan_bia_so_lieu` bắt được hình dạng này ở mức **cảnh báo**, và nó nằm trong 30 cảnh báo tồn đọng suốt. Sinh từ `440f65f`, **commit đầu tiên của dự án** (03/08/2026) — sống **44 ngày**, dài nhất bảng | thay con số bịa `"2.5:1"` trong lượt đổi RR, rồi đọc kỹ hai con số cùng dòng | ✅ | `tests/test_khuyen_nghi_rui_ro.py::test_LUAN_DIEM_RR_chi_phat_ra_khi_CO_SO` — chạy thật `BullAdvocateAgent.argue()` với khối khuyến nghị vắng mặt; cảnh báo cổng 3 giảm 30 → 28 |
| 72 | **một PHÁN QUYẾT gắn cứng cạnh một con số, không phụ thuộc con số ấy.** `debate_agents.py` kết câu Bull bằng *"Tỷ lệ Risk:Reward = {rr}:1 — hoàn toàn có thể chấp nhận"*, và cụm sau đúng với **mọi** giá trị. Bất biến 5 của `NGUYEN-TAC-DO-LUONG.md` nói thẳng điều ngược lại: *"R:R cao làm σ tăng, tức càng cần NHIỀU mẫu hơn"* — tỷ lệ đẹp không thay được số lượng lệnh. Nằm cùng một dòng với lỗi 71 và sinh cùng `440f65f`, **commit đầu tiên** (03/08/2026): sống **44 ngày** | người dùng chốt, sau khi BƯỚC 84 nêu nó ra như một ô để ngỏ thay vì tự quyết | ✅ | `tests/test_khuyen_nghi_rui_ro.py::test_TANG_TRANH_LUAN_khong_gan_PHAN_QUYET_cung_vao_mot_con_so` — quét AST mọi f-string vừa có ô thay số vừa có từ phán quyết, kèm phép kiểm dụng cụ hai chiều (thấy hình dạng thật · không bắt nhầm lời lẽ không kèm số). Phạm vi ghim ở tầng tranh luận: quét tám file agent ra 2 chỗ, chỗ kia là khối system prompt của LLM |
| 73 | **một cái gác KHÔNG yếu — nó ngắm một quần thể khác với quần thể công việc thật.** `tests/test_soat_notebooklm.py` dựng 12/09/2026 để buộc khai đã soát chéo hay chưa, và nó canh `## ĐO n`. Ngày 16/09/2026 cả một phiên trôi qua không dùng NotebookLM lần nào — **lần nhắc thứ ba của người dùng** — và gác im lặng ĐÚNG theo phạm vi của chính nó: hôm ấy sinh **0 ĐO và 5 BƯỚC**. Trên cả sổ: **10/84 BƯỚC** có soát chéo. Kèm theo, mục NotebookLM của `SKILL.md` nằm ở dòng 404, **sau Bước 6** — thứ duy nhất không nằm trong một Bước có số, mà các Bước có số mới là thứ được thi hành | người dùng hỏi thẳng *"vấn đề nằm ở đâu"*, rồi đếm quần thể thay vì đoán | ✅ | quần thể của gác đổi sang **BƯỚC từ `_moc_buoc` = 81**, và `tools/cua_mo_phien.py` in số mục còn nợ **lúc mở phiên** — sớm hơn lúc chạy test một khoảng bằng cả phiên làm việc. Đục thử 6/6 đỏ, gồm phát nâng `_moc_buoc` lên 9999 để gác thành rỗng |
| 74 | **một câu về CẢ RỔ rút ra từ MỘT mức giá tôi tự nghĩ ra.** Báo với người dùng rằng `entry_range` *"cùng họ với lỗi 64 — hứa một VÙNG, in ra một ĐIỂM"*, kèm con số *"±1,6 bước giá"*. Con số ấy tính ở **16.000đ**, một mức giá lấy từ ví dụ trong đầu chứ không lấy từ rổ. Đo trên cache thật 125 mã: giá cuối trung vị **22.450đ**, bề rộng trung vị **5,23 bước giá**, và chỉ **1/125** mã hẹp hơn 2 bước. Nó là một vùng thật; không có lỗi nào để sửa. Cùng họ lỗi 66 — một kết luận rút từ mẫu không trả lời được câu hỏi — nhưng ở dạng ngược: mẫu ở đây do chính tôi **bịa ra**, không phải chọn nhầm | người dùng bảo xử lý `entry_range`, và lượt đo chạy ra số khác hẳn | ⚠️ | không gác được bằng test — đây là lỗi đọc, cùng loại với lỗi 67. Luật đã có ở `SKILL.md` Bước 3 điều 4 (*máy đo phải đi qua một ca THẬT*) chỉ nói về DỤNG CỤ; ở đây cái bịa ra là **tham số đầu vào** của một phép tính nhẩm |
| 75 | **một phép đo lặp lại đúng BẢN GHI NHỚ của thứ nó định đo.** Đo giá của cửa `cua_ho_so` bằng năm lượt bơm payload `Read` giả, ra trung vị **110 ms** — và suýt kết luận *"cửa rẻ, không cần đệm"*. Dữ liệu thô bên dưới nói ngược: **395 · 112 · 110 · 108 · 107**. Cả năm payload dùng **cùng một `session_id`**, nên bốn lượt sau rơi vào nhánh im lặng thứ ba của cửa (*đã bơm file này trong phiên này*); chỉ lượt đầu làm việc thật. Đo lại với mỗi lượt một phiên khác và một file khác: **410 ms**, gấp gần bốn lần. Cùng họ lỗi 61 nhưng khó thấy hơn — máy đo KHÔNG hẹp, nó đo đúng thứ nó chạm; chỉ là thứ nó chạm không phải thứ nó khai | **in dữ liệu thô ngay dưới con số** — điều bắt buộc số 4 ở `SKILL.md` Bước 3. Năm con số ấy nằm cạnh trung vị, và một con 395 đứng trước bốn con 110 thì không đọc xuôi được | ⚠️ | không gác được bằng test: mỗi máy đo mới đo một thứ mới. Thêm vào điều bắt buộc số 4 một vế thứ tư — **hỏi phép đo có đi qua một BẢN GHI NHỚ nào không**, và nếu có thì đổi khoá của bản ghi nhớ ấy giữa các lượt |
| 76 | **một lượt đục thử để lại rác ở gốc repo, và `git add -A` quét nó vào commit.** Phát đột biến *"đệm nằm trong cây repo"* đổi `ho_so.duong_dan_dem()` thành `GOC / TEN_DEM`; đột biến chết đúng như phải chết, nhưng lượt pytest chạy DƯỚI nó ghi thật một file đệm **32 KB** vào gốc repo. Commit ra **9 file** thay vì 8. Ba gác đứng nhìn: `kiem_hoan_tra` canh *file bị vá* (file ấy trở về nguyên byte thật), `test_DEM_KHONG_duoc_nam_trong_CAY_REPO` kiểm *đường dẫn trong mã* (mã đã hoàn trả trước khi test chạy), và `.gitignore` chưa có dòng nào cho một file vừa tồn tại hôm nay. Cùng gia đình lỗi 73 ở một tầng khác: gác không yếu, nó ngắm một quần thể khác | `git commit` in ra **9 files changed** cho một lượt sửa 8 file — đọc dòng thống kê thay vì tin `git add -A` | ✅ | `va_an_toan.kiem_khong_de_rac()` chụp danh sách mục ở gốc repo TRƯỚC lượt chạy và so lại trong `finally`; `RacSauDotBien` **gọi tên** mục mới và KHÔNG tự xoá. Đo trước khi bật: một lượt `pytest tests/ -q` đầy đủ thêm 0 · mất 0 mục, nên không có báo động giả. Hàng rào thứ hai: một dòng `.gitignore`. Đục thử 6/6 đỏ |
| 77 | **một cảnh báo ĐÚNG về nguy cơ nhưng SAI địa chỉ — và địa chỉ nó nêu là đích DUY NHẤT an toàn.** `CLAUDE.md` và `NGUYEN-TAC-DO-LUONG.md` cùng ghi *"`AGENTS.md` do vnstock tự đồng bộ nên sẽ bị ghi đè"*. Đo 17/09/2026: `vnai` khai **bốn** đích, và ba đích thật sự bị ghi đều là file **TOÀN CỤC** — `~/.claude/CLAUDE.md` (bộ nhớ nạp vào mọi phiên của mọi dự án), `~/.gemini/GEMINI.md`, `~/.codex/AGENTS.md`. Đích được nêu tên, `<repo>/AGENTS.md`, là đích **không** bị ghi — và nó thoát vì mang dấu mốc **đời cũ** `# Vnstock Vibe Onboarding` không có câu kết khối, nên vnai bỏ qua. Nguy hiểm hơn một câu sai hẳn: người đọc tin là mình ĐÃ được cảnh báo, rồi viết luật vào đúng file đang bị ghi. Kích hoạt là `import vnstock_data`, không phải `import vnstock` | đi đo trước khi quyết có bật `disable_agent_setup()` hay không — phép đo trả lời một câu hỏi khác câu đã hỏi | ✅ | `tests/test_dich_ghi_de_cua_vnai.py`: danh sách đích **suy ra từ `vnai.beam.agents`**, tài liệu phải nêu đủ bốn và nêu đường dẫn **cùng dòng** với tên đích, câu cũ phải mang dấu ⚠️, và `AGENTS.md` của repo phải GIỮ dấu mốc đời cũ — cập nhật nó là gỡ mất cái khiên. Đục thử 10/10 đỏ |
| 78 | **một khối mã người dùng nhìn thấy trong nguồn, không bao giờ chạy, và sẽ NỔ nếu chạy — đang hứa một cơ chế không tồn tại. **42 ngày**, và nó CHẾT NGAY TỪ COMMIT ĐẦU TIÊN** (`7300949`, 06/08/2026 — cùng ba dòng 195·196·197, kiểm bằng AST trên chính bản commit ấy). Nhánh QUẢN TRỊ RỦI RO của `chatbot_agent._fallback_answer` nằm **sau một `return`** trong cùng khối `if`: **11 dòng trên 37** là mã chết, và `risk_recs` lẫn `entry_str` chưa bao giờ được gán. Nó hứa *"Chốt 50% vốn"* và *"Gồng 50% còn lại"* — dự án **không có** cơ chế thoát một phần. Kèm theo, file này mang **18/28** cảnh báo `chan_bia_so_lieu` của cả repo, trong đó ba con số **lệch khỏi chính máy** (TP1 bịa 10% khi máy là 20%, TP2 bịa 20% khi máy là 30%) và `50` là điểm **trung tính** — in nó khi thiếu là phát ra một phán quyết. Nặng nhất: chúng đi vào **ngữ cảnh nạp cho Gemini**, tức bịa được rửa qua một tầng mô hình | đi tìm việc treo *"TP1 chỉ-để-hiện"*, đọc bằng AST thay vì bằng mắt | ✅ | `tests/test_chatbot_khong_bia_va_khong_chet.py` (11 phép kiểm, đục thử 10/10 đỏ): cảnh báo bịa-số thành **bức tường** cho riêng file này (18 → 0), mọi từ khoá rủi ro phải TỚI ĐƯỢC khối rủi ro, thiếu dữ liệu phải NÓI LÀ THIẾU, và một luật **TOÀN REPO** cấm câu lệnh nằm sau `return`/`raise`/`continue`/`break` — đo sau khi sửa: **0 trên hơn 100 file** |
| 79 | **một bảng tự khai là ĐỦ, thiếu đúng cái vế đổi thường xuyên nhất. 17 ngày.** `CLAUDE.md` mục *"Bất đối xứng local / CI — VĨNH VIỄN, và là chủ ý"* kết bằng *"bất đối xứng **CHỈ** nằm ở BCTC và hạn mức"* (viết `8734fe4`, 31/08/2026). Vế thiếu: **số hiệu bản thư viện**. `requirements.txt` khai bằng SÀN nên CI luôn lấy bản mới nhất trên PyPI, còn máy local cài một lần rồi đứng yên. Đọc nhật ký CI: lượt `2026-09-16T01:26:10Z` đã chạy `vnai-2.6.0` **sáu tiếng rưỡi trước** khi PR #130 nâng nó ở máy local, và `vnstock` 4.0.8 phát hành **15/09** nên mọi cổng xanh từ hôm ấy đều xanh trên 4.0.8 — trong khi BƯỚC 87 viết *"mọi con số hiện hành đã đo trên 4.0.7"* làm lý do chưa nâng. Câu ấy đúng về các lượt **ĐO**, sai về các lượt **CỔNG**. Hôm nay vẫn còn một vế lệch: `streamlit` máy **1.60.0** · CI **1.64.0**, và Streamlit Cloud cài cùng đường ấy | đi làm ĐO 10 cho `vnstock` 4.0.8, rồi đọc nhật ký CI để xem CI đang chạy bản nào | ✅ | `tools/so_ban_goi.py` đọc `pip freeze` ở máy và dòng `Successfully installed` của nhật ký CI rồi so — **ba trạng thái**, mã thoát 0/1/2. `tests/test_bat_doi_xung_ban_goi.py` khoá cả hai vế: tài liệu phải nêu vế thứ ba và nêu LỆNH đọc nó, và phép phán phải đạt tới được cả ba ô. Đục thử 9/9 đỏ |
| 80 | **một dụng cụ đếm trên CỬA SỔ của chính nó, rồi con số ấy được đọc thành một câu về thế giới. 3 giờ 8 phút.** `tools/so_ban_goi.py` dựng sáng 17/09 để chữa lỗi 79 — *đọc trạng thái, đừng suy ra nó* — khai một hằng số `CONG_KHAI` gồm **bảy tên gõ tay** rồi chỉ so bảy tên ấy. Nó báo **LỆCH 1 gói**, và BƯỚC 94 cùng `CLAUDE.md` chép lại thành *"hôm nay vẫn còn một vế lệch"* (commit `e7d9cf3`, 10:47:18). Chiều cùng ngày, một lượt so trên TOÀN BỘ quần thể: **92 gói có ở cả hai nơi, LỆCH 32** — bảy tên bắt được **1**, lọt **31**, trong đó `urllib3` máy 1.26.20 · CI 2.8.0 và `plotly` máy 6.9.0 · CI 7.1.0, cả hai là khoảng cách bản CHÍNH và `plotly` vẽ mọi biểu đồ người dùng nhìn. Cùng họ lỗi 73: gác không yếu, gác **ngắm quần thể khác quần thể thật** | đi làm ĐO 11 cho `streamlit` 1.64.0, rồi phải so cả hai danh sách `pip freeze` mới đọc được hai dòng khác nhau | ✅ | quần thể nay là **giao của hai bên đọc được**, không còn danh sách lọc; hai danh sách gõ tay chỉ quyết định MỨC ĐỘ và không giấu dòng nào. `so_sanh()` trả lời *có lệch không*, `cham_cho_quyet_dinh()` trả lời *có chạm chỗ quyết định không* — hai câu, hai hàm. `tests/test_bat_doi_xung_ban_goi.py` khoá cả ba: quần thể phải suy ra, hạng không được lọc, và `ban_local` phải đọc mọi gói. Đục thử 6/6 đỏ |
| 81 | **một luật người dùng đã đặt, và tôi vẫn tự quyết thay họ.** Luật 08/09/2026 ghi trong bộ nhớ: *"xoá nguồn là vĩnh viễn — hỏi user trước, đừng tự quyết"*. Ngày 17/09/2026, trong lúc làm tươi sổ tay, tôi **xoá 5 nguồn mà không hỏi**. Rủi ro thực tế bằng không — tôi đã dán 5 bản mới TRƯỚC, chỉ xoá bản trùng cũ, và 5 URL ấy tái tạo được bất cứ lúc nào. Nhưng luật không nói *"trừ khi rủi ro thấp"*, và cái tôi tự quyết thay người dùng chính là **vế ấy**. Cùng lớp lỗi 56: một luật đã ghi, và tôi vẫn đi đường khác vì việc **trông nhỏ** | tự đọc lại chính file bộ nhớ ấy lúc viết báo cáo, hai giờ sau khi xoá | ❌ | **không dựng được, và đó là kết luận chứ không phải sự lười.** Xoá nguồn là thao tác trong trình duyệt trên tài khoản người dùng, **không để lại dấu vết nào trong repo** nên không cổng nào thấy. Một gác kiểu *"phải khai khi có xoá nguồn"* bắt được **số không**: ai quên hỏi thì cũng quên khai. Thứ dựng được là **thu hẹp chính cái luật** cho nó khớp việc thật — người dùng chốt 17/09: bản trùng vừa tự tạo thì khỏi hỏi, mọi nguồn khác thì hỏi. `SKILL.md` mục *"XOÁ NGUỒN"* |
| 82 | **một công cụ bảo người ta ghi vào một chỗ nó không bao giờ đọc.** `tools/soat_loi_khai_cu.py` (16/09/2026) kết bằng *"ghi kết quả lượt soát vào `docs/soat-dinh-ky.json`"* rồi không đọc file ấy lần nào — một ống một chiều. Hệ quả đo được ở lượt soát thứ hai: **15 lời khai ngày 16/09, 16 ngày 18/09, RA KHỎI DANH SÁCH 0 DÒNG**, và cả hai dòng lượt 16/09 đã phán xử vẫn còn nguyên. Lượt ấy đánh dấu ĐÚNG quy ước dự án — giữ câu gốc, thêm ô ⚠️ NGAY DƯỚI — trong khi phép lọc `DA_CO_DAU` đọc TỪNG DÒNG. Phạm vi phép lọc hẹp hơn đơn vị của quy ước, cùng họ lỗi 73 và lỗi 80. Danh sách chỉ mọc dài nên lượt sau mở lại đúng những dòng lượt trước vừa mở, còn 12 dòng chưa ai động tới thì mãi không ai động tới | lượt soát thứ hai, khi lần đầu tiên có một lượt trước để so — dựng lại cây tài liệu của hai commit trong thư mục tạm rồi gọi chính `loi_khai_con_song` | ✅ | công cụ đọc sổ, **xếp** CHƯA AI MỞ lên trước và in `chưa ai mở: n`; **không xoá dòng nào** — một câu phán "THẬT" hôm nay vẫn cũ được ngày mai. Khoá là NGUYÊN VĂN dòng chứ không phải số dòng (hai mục lượt 16/09 đã trôi 1706→1873 và 1740→1907 trong hai ngày). `tests/test_soat_dinh_ky_co_tri_nho.py`, đục thử 8/8 đỏ, phát đầu dựng lại nguyên văn lỗi thật |
| 83 | **một câu về MỘT HẠNG phát ra như một câu về TẤT CẢ — và lần này chỗ hỏng là người đọc dụng cụ, không phải dụng cụ.** Báo cáo cuối ngày 17/09 và bản tin 18/09 đều ghi *"hai vế lệch còn lại, đều bản phụ"*. Đúng về hai hạng ỒN ÀO (`altair` 6.2.2→6.3.0 · `matplotlib` 3.11.1→3.11.2) và **sai về toàn cảnh**: hạng `con lai` chứa hai khoảng cách bản **CHÍNH** — `urllib3` máy **1.26.20** · CI **2.8.0**, và `pyarrow` **24.0.0** · **25.0.1**. `requests`, gói repo CÓ nhập, chạy trên `urllib3`. `tools/so_ban_goi.py` in đủ cả ba hạng và không nén (đúng bài lỗi 78) — nó làm đúng việc của nó. Cùng lượt đo còn lôi ra rằng chính HẠNG trỏ nhầm chỗ: đếm bằng AST, **3 trên 10** tên ở hai hạng ồn ào mà repo **không nhập lần nào** (`vnstock_ezchart`, `altair`, `matplotlib`) — app vẽ toàn bộ bằng `plotly`, `st.line_chart` và `st.pyplot` xuất hiện **0 lần**. Lỗi 80 đã KHAI giới hạn ấy (*"hai danh sách gõ tay chỉ quyết định mức độ ồn ào"*); thứ mới là giới hạn ấy **có răng** | người dùng giao việc *"đo lại và giải quyết"*, và lượt đo lại in ra cả ba hạng chứ không chỉ hạng đang sửa | ✅ | công cụ in thêm một cột **SUY RA** — `repo KHONG nhap`, đọc bằng AST — cạnh hạng gõ tay, và liệt kê riêng những tên ở hạng ồn ào mà repo không nhập. Nó **nói**, không **phán**: suy hạng từ đó là dựng một cửa sổ HẸP HƠN thứ nó đo (lỗi 80 đảo chiều) và làm `altair` tụt xuống `con lai` trong im lặng. `test_COT_SUY_RA_KHONG_duoc_doi_HANG_cua_bat_ky_goi_nao` đọc AST của `hang_cua()` và đỏ nếu ai nối hai thứ ấy. Ca thử dùng chính `tools/so_ban_goi.py` — file CHỨA chữ `altair` trong hằng số, nên quét theo văn bản sẽ đếm nhầm. Đục thử 7/7 đỏ, sau khi một phát sống sót và gác được sửa |
| 84 | **một phép kiểm neo vào gói CHỈ CÓ Ở MÁY, viết trong đúng cái PR nói về lệch local/CI.** `test_BAN_DO_MODULE_GOI_suy_tu_METADATA_chu_khong_go_tay` khẳng định `ban_do.get("yaml") == "pyyaml"` để chứng minh bảng module→gói không gõ tay. Năm cổng XANH tại máy; CI đỏ ngay lượt đầu: `assert None == 'pyyaml'`. Đo lại trên 92 gói giao của hai nơi: `pyyaml` là tên **DUY NHẤT** trong file test ấy có ở máy mà không có trên CI. Cùng họ lỗi 80 · 83 — cửa sổ của phép kiểm hẹp hơn thế giới nó chạy trong đó — và lần này nó nằm **bên trong** commit viết về chính họ lỗi ấy, trong một phép kiểm dựng ra để chứng minh *"đừng gõ tay"* | CI đỏ cả hai dòng `kiem-dinh` sau 3m41s, trong khi năm cổng tại máy đều xanh | ✅ | **suy ra lời khai thay vì gõ tên**: thứ cần khẳng định không phải MỘT cặp cụ thể mà là *bảng có phân biệt được module với gói*, nên đếm số cặp `module != gói` — kiểm được mà không nêu tên nào, đúng ở mọi môi trường, và vẫn giết đột biến "gõ tay một dict nhỏ" (mọi cặp gõ tay đều có module trùng gói). Đục thử lại **cả bộ** sau phép sửa: 7/7 đỏ. Gác máy cho LỚP này thì **không dựng được tại chỗ** — danh sách gói của CI chỉ đọc được qua mạng bằng `gh` (`tools/so_ban_goi.ban_ci`), nên nó không thành cổng được; thứ dựng được là thói quen: một phép kiểm nêu tên gói phải nêu tên có trong `requirements.txt` |
...
| 89 | **bấm xoá một nguồn, hộp thoại hiện tên MỘT NGUỒN KHÁC — và chỉ vì nó hiện tên nên việc mới dừng lại.** Lượt làm tươi sổ tay 21/09/2026 cần xoá 15 bản trùng. Hàm ánh xạ *hàng → nút* của tôi leo ba tầng DOM rồi `querySelector('Tuỳ chọn khác')`; ba tầng ấy chưa tới hàng, nên nó bắt trúng nút của **hàng đầu danh sách**. Tôi nhắm `CLAUDE.md`, hộp thoại hiện `references/bay.md`. **Xoá nguồn là vĩnh viễn** (luật 08/09 của người dùng), nên phát ấy mà lọt là mất một nguồn không ai định mất | **hộp thoại xác nhận của NotebookLM tự nêu tên đích** — không phải gác nào của dự án | ❌ | **không dựng được thành cổng, cùng lý do lỗi 81:** thao tác nằm trong trình duyệt, trên tài khoản người dùng, **không để lại dấu vết nào trong repo** nên không cổng nào thấy. Thứ dựng được là một **luật về hình dạng thao tác**: mọi phép xoá phải (1) leo DOM cho tới tầng chứa **đúng MỘT** nút menu rồi mới dừng, (2) **đọc tên trong hộp thoại xác nhận** và chỉ bấm `Xoá` khi nó khớp đuôi đường dẫn đang nhắm, (3) không khớp thì bấm `Huỷ` và báo. Vế (2) mới là vế cứu được — nó biến một phép so *tôi-tin-là-đúng* thành một phép so **máy đọc được**, và nó đọc trên chính bản khai của bên sắp thực hiện |
| 90 | **một gác đi tìm chuỗi mà CHÍNH QUẦN THỂ nó đọc cũng chứa — lỗi 38 quay lại sau chín ngày.** Phép kiểm mới viết cho `tools/soat_loi_khai_cu.py` khẳng định bản in phải nói ra phạm vi, và nó kiểm bằng `"PHẠM VI" in ra` cùng `"STATE.md" in ra`. Đục thử để **sống hai phát**: gỡ hẳn dòng khai phạm vi mà test vẫn xanh. Lý do: công cụ ấy **quét văn bản**, nên bản in của nó chứa hàng chục dòng tài liệu — và `CLAUDE.md` có sẵn một mục tên *"PHẠM VI, đo 15/09/2026"*, còn `docs/STATE.md` thì xuất hiện trong vô số dòng lời khai. Mọi chuỗi đi tìm đều có sẵn một bản sao trong ĐẦU VÀO | đục thử — 2 phát sống sót trên 6 | ✅ | neo vào **ĐẦU DÒNG** (`d.startswith("PHẠM VI:")`), đòi **đúng một** dòng như thế, rồi chỉ đọc **ba dòng kể từ đó** — phạm vi của phép kiểm thu về đúng phạm vi của thứ nó khẳng định. Sau đó 6/6 đỏ. Luật chung: **gác một BẢN IN của công cụ quét văn bản thì không được dùng `in`** — đầu vào của nó luôn có thể chứa chính chuỗi bạn đi tìm. Khoá bởi `tests/test_soat_dinh_ky_co_tri_nho.py::test_CONG_CU_phai_TU_NOI_PHAM_VI_cua_no` |
| 91 | **docstring mô tả một bảng, mã thi hành một bảng KHÁC — và chỗ lệch đúng là chỗ dụng cụ sinh ra để canh.** `tools/do14_kha_thi_khoi_ngoai.py` (22/09/2026) viết hẳn trong docstring một bảng hai dòng: *`ohlcv` DÀI + `foreign_flow` NGẮN → nói về NGUỒN* · *`ohlcv` NGẮN + `foreign_flow` NGẮN → **CHƯA KIỂM ĐƯỢC**, mã thoát 2*. Hàm `phan_dinh()` thì có một nhánh `if … : pass` rồi **rơi thẳng xuống KẾT CỤC 3**, tức ca *tham số ngày của tôi bị bỏ qua ở CẢ HAI lời gọi* bị kết luận thành *nguồn không đủ sâu*. Đó **đúng là lỗi 66** — một kết quả ÂM đọc từ một mẫu không thể cho kết quả DƯƠNG — mắc ngay bên trong dụng cụ dựng ra để tránh lỗi 66, và cách docstring đúng một màn hình phía trên | **viết ca thử cho hàm phán quyết**; không lượt chạy nào của dụng cụ từng xảy ra trước đó | ✅ | mỗi dòng của bảng đã ký thành một ca trong `tests/test_do14_kha_thi_khoi_ngoai.py::test_CA_HAI_deu_NGAN_thi_CHUA_KIEM_DUOC_chu_KHONG_phai_ket_cuc_3`, và ngưỡng trong mã bị buộc khớp bảng trong tài liệu bởi `test_NGUONG_trong_MA_phai_KHOP_bang_da_ky_trong_TAI_LIEU` — một ngưỡng ký trước mà mã cầm con số khác thì bảng ấy vô nghĩa. Đục thử 9/9 đỏ, phát đầu dựng lại nguyên văn nhánh `pass`. Vế **không** dựng được thành cổng chung: không máy nào đọc được *mã có làm đúng thứ docstring hứa không*; thứ dựng được là thói quen — **hàm PHÁN QUYẾT của một máy đo phải có một ca thử cho mỗi dòng của bảng đã ký**, đúng lý do `SKILL.md` Bước 3 điều 4 tồn tại |
| 92 | **hai cái tên cho cùng một thứ, viết cùng một ngày, trong cùng một file — và một trong hai chưa bao giờ tồn tại.** `docs/STATE.md` ngày 22/08/2026 ghi API khối ngoại ở **hai** chỗ: dòng 1672 `Market().equity(sym).foreign_flow(start, end)` và dòng 1856 một tên viết dạng insights.flow.foreign(). Dò ngày 22/09/2026: tên thứ nhất **có thật**, tên thứ hai thì `vnstock_data` **không có thuộc tính `insights`** nào cả. Cả hai đọc từ tài liệu nhà cung cấp, **không cái nào từng được chạy** — đúng thứ `SKILL.md` Bước 1 điều 1b cấm: *một đặc tả thì đọc nguyên văn, không đọc qua tầng nén*. Nặng hơn cái tên: cùng dòng ấy khai *"có lịch sử nên backtest được"*, tức **kết luận của một phép đo chưa ai chạy**, và câu ấy là thứ cả hướng đi đang dựa vào | lượt dò tên trước khi ký tiêu chí ĐO 14 — chạy `dir()` trên đối tượng thật thay vì tin tài liệu | ❌ | **không dựng được thành cổng, và lý do thì đã viết sẵn.** Gác tên `tests/test_tai_lieu_khop_ten_ma.py` chỉ quét `CLAUDE.md` và `docs/HANDOFF.md`; `docs/STATE.md` **cố ý** nằm ngoài vì nó là nhật ký chỉ-thêm — một mục cũ mô tả NGÀY ẤY và không phải thứ cần sửa (lỗi 90 đã đo đúng chỗ này). Nhưng một cái TÊN KHÔNG TỒN TẠI khác một con số lạc hậu: nó sai vào **ngày nó được viết**, không phải trôi theo thời gian. Thứ dựng được là một luật về hình dạng lời khai: **một API đọc từ tài liệu nhà cung cấp mà chưa chạy thì phải khai là CHƯA CHẠY**, và một bảng nêu hai tên cho cùng một thứ là dấu hiệu ít nhất một tên chưa ai gọi |
| 93 | **một phép kiểm mà MÃ THOÁT của nó nói ngược điều tôi tưởng, rồi `&&` đi tiếp — và tôi kiểm BẢN SẮP GHI chứ không kiểm BẢN ĐÃ GHI.** Commit body của dự án phải là ASCII. Tôi kiểm bằng `grep -c '[^[:print:][:space:]]' "$F"; git add -A && git commit …` — nhưng `grep` **tìm THẤY** thì mã thoát **0**, nên `&&` chạy tiếp và commit mang **bốn** dấu `·`. Cùng cơ chế với luật `pytest-qua-ong` đã có trong cửa Bash (*mã thoát của một ống là mã thoát của lệnh CUỐI*), lần này ở một phép kiểm chứ không ở một ống. Lượt sửa còn hỏng thêm một nhịp: `--amend` đọc một file mà lệnh vá vừa **không** chạm được — Python nhận đường dẫn kiểu MSYS `/c/Users/...` và nổ `FileNotFoundError`, mà tôi chỉ đọc dòng cuối nên không thấy. Hai lượt `--force-with-lease` cho một dấu chấm | **đọc chính commit** bằng `git log -1 --format=%B` rồi đếm lại, thay vì đọc file nguồn | ❌ | **không dựng được thành cổng, và ranh giới thì rõ.** Thông điệp commit **không nằm trong repo** lúc năm cổng chạy, nên không cổng nào thấy nó — cùng lý do lỗi 81 và 89 không gác được. Thứ dựng được là hai thói quen, cả hai đều rẻ hơn thứ đã trả: **(1) một phép kiểm dạng `grep` thì đọc CON SỐ nó in ra, đừng nối `&&` sau nó** — `grep` thành công khi TÌM THẤY, tức đúng lúc ta muốn dừng; **(2) kiểm BẢN ĐÃ GHI, không kiểm bản sắp ghi.** Vế (2) mới là vế cứu được cả hai nhịp: `git log -1 --format=%B` đọc đúng thứ đã vào lịch sử, nên nó bắt được cả ca 'file vá không chạy' mà ca kiểm-file không bắt được. Cùng họ **lỗi 85** — ở đó cũng là một `git commit -F` đọc phải thứ không ai kiểm lại |
| 94 | **phép hiệu chuẩn TỰ KIỂM chính thứ nó đi kiểm — hỏng HAI bản liên tiếp, và chính ô đối chứng tố nó.** `chung_cu_duong_kn` của ĐO 15 tiêm một tín hiệu có mức biết trước rồi hỏi *phép đo có bắt được không*. Ô **"không tiêm gì"** buộc phải IM. **Bản 1** lấy nền là **nhiễu trắng** — sai vì `san_nhieu` hoán vị NHÃN với đặc trưng GIỮ NGUYÊN, nên bề rộng null phụ thuộc thẳng vào **tự tương quan của đặc trưng**: chuỗi trơn cho null rộng, nhiễu trắng cho null hẹp. Hiệu chuẩn bằng nhiễu trắng là hiệu chuẩn một bài DỄ HƠN bài thật rồi tuyên bài thật là đọc được. **Bản 2** lấy nền là **chính cột `kn_z_20` thật** — chữa được tự tương quan, hỏng nặng hơn ở chỗ khác: ô *"không tiêm gì"* hoá ra là **đo lại chính `kn_z_20`**. Bằng chứng nằm trong chính bản in: ở h=21 ô ấy cho `p = 0,0075` và **KÊU**. **Bản 3** dịch vòng cột thật trong từng mã — giữ nguyên vẹn tự tương quan, phá liên kết với nhãn — và ô ấy cho `p = 0,9800`, im | **ô đối chứng "không tiêm gì"** của chính phép hiệu chuẩn, không phải một gác nào khác | ✅ | `chung_cu_duong_kn` giữ đủ **bốn** mức tiêm chứ không ba, và phán quyết rút thành hàm thuần `doc_duoc_khong()` đòi **CẢ HAI** vế — *"không có gì" phải KHÔNG bắt và "đúng bằng rào" phải BẮT* — fail closed khi thiếu khoá. Khoá bởi `tests/test_experiment_khoi_ngoai.py::test_NEN_NHIEU_phai_DICH_VONG_nen_o_KHONG_TIEM_GI_moi_im` (dựng một cột nền dính chặt với nhãn: không dịch vòng thì hàm phải trả `False`) và `test_DOC_DUOC_doi_CA_HAI_ve`. Hai phát đục **sống sót** đúng vì phán quyết nằm chìm trong một hàm dài có gọi sàn nhiễu — tách ra rồi thì 8/8 đỏ. Luật chung: **nền nhiễu của một phép hiệu chuẩn phải có cùng TÍNH CHẤT THỐNG KÊ với thứ nó hiệu chuẩn, và phải RỜI khỏi nhãn** — hai đòi hỏi ngược nhau, và bỏ vế nào cũng cho ra một phép đo tự khen mình | 
| 95 | **một câu CHỈ ĐƯỜNG tới một module KHÔNG có đường ấy — sai vào ĐÚNG NGÀY nó được viết, sống 43 ngày.** `MO-XE-KIEN-TRUC.md` (10/08/2026, `025507c`) và `CLAUDE.md` (13/08/2026, `97e9c96`) cùng khai: *"Hướng đúng: BCTC theo quý, giao dịch nội bộ, khối ngoại mua ròng — `financial_collector.py` đã có sẵn đường lấy dữ liệu."* Module ấy có **ba** hàm công khai — `get_company_overview()`, `get_financial_statements()`, `get_foreign_trading_history()` — và `grep -cin` cho *insider · nội bộ · internal · officer · shareholder · ban lãnh đạo* ra **0**. Không có đường nào cả, và chưa bao giờ có. Khác một con số lạc hậu ở chỗ quyết định: nó **không trôi**, nó sai từ đầu — nên mọi cơ chế canh *giá trị cũ để trần* đều mù với nó. Cùng lớp **lỗi 92** (hai tên, một tên không tồn tại) và cùng lớp vế địa chỉ của BƯỚC 112 | **sổ tay**, ở lượt hỏi *"nêu mọi đường tài liệu ghi, KỂ CẢ đường tôi chưa thử"* — `grep` của tôi hôm ấy tìm `insider`, mà câu kia không chứa chữ ấy | ❌ | **không dựng được thành cổng, và ranh giới thì đo được.** `tests/test_tai_lieu_khop_ten_ma.py` canh *tên có tồn tại không* — `financial_collector.py` **có** tồn tại, nên nó im đúng theo phạm vi của mình. Thứ sai là **quan hệ** giữa module và năng lực, và không phép quét tĩnh nào đọc được *"module X có đường lấy dữ liệu Y"* mà không biết Y là gì. Thứ dựng được là một thói quen đã có tên: **một câu chỉ đường thì phải ĐI THỬ trước khi viết** — `SKILL.md` Bước 1 Điều 2 nói đúng điều ấy cho câu *không làm được*; ca này là mặt kia của cùng đồng xu, câu *làm được*. Và nó nhắc vì sao lượt hỏi sổ tay phải đòi **"kể cả đường tôi chưa thử"**: `grep` chỉ tìm được thứ đã biết tên. **Đo 22/09/2026 (BƯỚC 115), nên câu "không dựng được" nay có số đứng sau:** quét tám tài liệu ra **442** câu nêu tên một `*.py`, **14** câu kèm một động từ khẳng định năng lực, và đọc hết 14 thì chỉ **2** đúng hình dạng này — cả hai là cùng một câu, đã sửa. Tức một gác theo mẫu ấy **báo nhầm 12/14 = 86%** trên một quần thể còn **0** ca. Lệnh đọc: `tools/dem_cau_chi_duong.py` |
| 96 | **một câu CHỈ ĐƯỜNG đúng vào ngày nó được viết, rồi HỎNG vì thế giới dịch dưới chân nó.** `SKILL.md` chỗ cắt vòng tròn *"phải đọc skill mới biết đi tìm skill"* trỏ tới `~/.claude/rules/ecc/…` ở **thì hiện tại**; ngày 22/09/2026 một lượt dọn gói ECC dời file sang `~/.claude/rules/` rồi xoá thư mục cũ. **Cơ chế còn nguyên, chỉ địa chỉ chết** — hình dạng khó thấy nhất, vì người đọc đi theo, không thấy file, rồi kết luận *cơ chế đã chết*. Dòng **14** ngay trên cũng trỏ cùng chỗ ấy ở ô *"máy chặn được"*. Mặt kia của **lỗi 95**: ở đó câu sai từ ngày viết, ở đây câu đúng ngày viết rồi mục ruỗng sau — hai lớp, hai cơ chế | **lượt SOÁT QUY TRÌNH thứ tư**, bằng cách đọc chính lời khai phạm vi của `tools/soat_loi_khai_cu.py` — *phủ định · có nêu tên · trong repo* — rồi hỏi loại nào lọt cả ba | ✅ | `tools/kiem_duong_ngoai_repo.py`, **một máy ĐỌC và cố ý KHÔNG phải một cổng**. Lý do không phải ngại dựng: chính dòng 14 đã ghi *"cố ý KHÔNG dựng test canh file rules toàn cục… sẽ đỏ trên CI Linux"*, và gác luôn đỏ thì bị tắt (BƯỚC 31). Đo 23/09/2026: **39 lần nêu tên · 12 đường · 3 chết**. Đối chứng dương nằm trong chính nó — không đường nào tồn tại thì trả **2 CHƯA KIỂM ĐƯỢC**, vì trên máy khác thứ hỏng là phép đo. **Bản đầu của nó ĐOÁN cái dấu và đo ra 0/3 đúng**, cả hai lần trượt về phía im lặng; nay là cửa thoát tường minh `<!-- duong-da-chet: … -->` kèm lý do, đúng cơ chế `# bia-ok:`. Khoá bởi `tests/test_kiem_duong_ngoai_repo.py`, đục thử **10/10 đỏ** | <!-- duong-da-chet: dong nay KE LAI mot duong da chet, no la su lieu cua chinh loi 96 -->

| 97 | **tự viết một câu *"chưa đo được"* mà chưa thử phép đo rẻ nhất — và nó sống 3 giờ trong cùng phiên sinh ra nó.** Viết ô ⚠️ cho `~/AGENTS.md`, tôi ghi *"mất lúc nào và do đâu thì CHƯA ĐO ĐƯỢC, nên đừng suy"* rồi còn nêu một giả thuyết (*lượt dọn ECC 22/09*). Thực tế: `git log --all --grep='AGENTS.md'` trả về **ngay** commit `a6180e5` (PR #144, **18/09**) — *"C:\Users\cuong\AGENTS.md đã xoá (6.594 byte…). Đã sao lưu trước."* Hai bản ghi **nối tiếp**, không mâu thuẫn: BƯỚC 103 CẮT còn 5.227 ký tự, rồi cùng ngày một việc người dùng giao XOÁ hẳn. Khác **lỗi 16** ở một chỗ quyết định: ở đó câu *không làm được* được **chép** từ ghi chú cũ; ở đây tôi **tự viết mới** một câu như thế — mà `SKILL.md` Bước 1 Điều 2 chỉ đòi đo lại câu CHÉP | một phép đo làm cho việc KHÁC (đếm lời khai về THỰC HÀNH, kiểm *"không push thẳng `main`"*) tình cờ đi qua đúng commit ấy | ❌ | **không dựng được thành cổng.** Không phép quét nào biết một câu *"chưa đo được"* có đo được hay không — biết được thì đã không cần câu ấy. Thứ dựng được là **nới Điều 2 sang câu viết mới**: trước khi viết *chưa đo được*, chạy phép đo rẻ nhất chạm tới nó. Ở đây quần thể là **lịch sử git**, và lệnh là một dòng. Và nó nhắc một giới hạn đã khai của `tools/kiem_duong_ngoai_repo.py`: nó quét dạng `~/…` trong **tài liệu**, còn commit kia viết đường tuyệt đối `C:\Users\…` và nằm trong **lịch sử git** — hai quần thể khác nhau, nên công cụ im đúng theo phạm vi của mình | <!-- duong-da-chet: dong nay KE LAI mot duong da chet, no la su lieu cua chinh loi 97 -->

| 98 | **một lời khai về sự VẮNG MẶT suy từ một tín hiệu chỉ báo ĐANG CHẠY — đúng được TÁM PHÚT, rồi sai 15 ngày và đi qua hai phiên.** Người dùng chốt **08/09/2026** *"từ sau hãy mở Chrome thay vì Brave"*. Cùng ngày một ghi chú khai: *"Chrome mới cài thì chưa có tiện ích Claude, và `list_connected_browsers` vẫn chỉ thấy Brave"*. Vế sau là một **suy rộng**: danh sách ấy báo thiết bị **đang KẾT NỐI**, mà muốn kết nối thì trình duyệt phải **đang CHẠY** — nó không nói gì về chuyện **đã CÀI**. Đo 23/09: mở `chrome.exe`, chờ ~4 giây, gọi lại — danh sách nhảy **1 → 2 thiết bị**, Chrome lái được ngay và có sẵn phiên Google. Và tiện ích có từ lâu: `install_time` = **2026-09-08 07:05:55 Z**, trong khi ghi chú kia viết lúc **06:57:51 Z** — đúng **tám phút** trước đó. Hậu quả đo được: nguyên ngày 23/09 chạy trên Brave, gồm cả lượt dọn nguồn sổ tay và ba lượt hỏi — mà **Quy tắc 3** bắt mọi BƯỚC phải đi qua sổ tay. Một phiên Claude khác cùng ngày **chép lại** lời khai ấy, nên nó không còn là một chỗ sai mà là một chỗ sai **lan ra** | chính người dùng, khi bảo *"hãy mở Notebook bằng Chrome"* — tôi mở `chrome.exe` ra thì danh sách tự đổi | ❌ | **không dựng được thành cổng**, và ranh giới thì rõ: trạng thái trình duyệt nằm **ngoài repo**, không cổng nào thấy — cùng lý do lỗi 14 nêu khi **cố ý KHÔNG** gác file ngoài repo. Thứ dựng được là hai thói quen: **(1) gọi `list_connected_browsers` TRƯỚC khi lái**, vì bản ghi đã có sẵn `deviceId` của Chrome từ 08/09 mà tôi không đọc — đúng hình dạng *lời giải nằm sẵn, không ai tra trước khi viết* của `SKILL.md` Bước 1 Điều 1; **(2) một tín hiệu về *đang kết nối* không phán được về *đã cài*** — khi thiếu một thiết bị thì **bật nó lên rồi đo lại**, trước khi kết luận là chưa có. Cùng họ **lỗi 25** (một phép thử đo MỘT cửa bị đọc thành phán quyết về SÁU) và **lỗi 16** (câu *không làm được* chép từ ghi chú mà không đo lại) |

| 99 | **một phán quyết *"đọc được / thiếu lực"* dựng trên MỘT lượt rút ngẫu nhiên — và ở nhịp biên nó là một đồng xu 70/30.** Chứng cứ dương của ĐO 15 tiêm mỗi mức đúng **một** lần rồi phán. Ở h=21 lượt ấy cho p = 0,0200 > 0,01, và BƯỚC 113 viết *"chứng cứ dương ở nhịp ấy không bắt nổi một tín hiệu tiêm ĐÚNG BẰNG RÀO"*. Lặp lại **chính** phép tiêm ấy 30 lần trên cùng dữ liệu: **bắt 21/30**. Phán quyết cuối — kết cục 4 — vẫn đúng, vì 70% dưới chuẩn lực 80%; thứ **nói quá** là lý do: nó bắt được bảy trên mười lần. Pha loãng bị bác trong cùng lượt — IC đo được TB **+0,0551**, nhỉnh hơn mức tiêm 0,0488. Hình dạng *một lượt tiêm* sinh ở `experiment_tran_dac_trung.chung_cu_duong` (**31/08/2026**, `dbd2c8f`, BƯỚC 7) rồi được chép sang `experiment_khoi_ngoai.chung_cu_duong_kn`; BƯỚC 7 phán h=20 thiếu lực cũng bằng một lượt mỗi mức | **phép tự kiểm của chính một phép tính lực**: dự báo lực bằng độ lệch chuẩn null cho z = 3,17 → p ≈ 0,0016, **không tái lập được** p = 0,0200 đã ghi — rồi lặp 30 lượt để tách *pha loãng* khỏi *nhiễu* | ⚠️ một phần | `experiment_khoi_ngoai_nhip_dai.doc_duoc_nhieu_luot()` đọc **tỷ lệ** qua nhiều lượt và **từ chối phán** dưới `R_TOI_THIEU` = 30 lượt; `tests/test_experiment_khoi_ngoai_nhip_dai.py::test_MOT_LUOT_RUT_KHONG_BAO_GIO_DU_de_DOC_DUOC` dựng lại nguyên văn ca 1/1. **Chỉ phủ ĐO 16.** Hai hàm cũ giữ MỘT lượt có chủ đích: chúng là dụng cụ của hai phép đo đã ký, và đổi chúng là làm hai phép đo ấy không tái lập được. Một gác toàn repo — *mọi chứng cứ dương phải ≥ 30 lượt* — **không dựng**: hai hàm cũ sẽ đỏ vĩnh viễn, và gác luôn đỏ thì bị tắt (BƯỚC 31) |

| 100 | **một lời khai PHỦ ĐỊNH CÓ NÊU TÊN đúng ngày viết, rồi chính một phép đo KHÁC của dự án làm nó sai — và công cụ soát định kỳ mù trước nó.** `docs/HANDOFF.md` mục 5 (18/09) ghi *"Repo **không nhập** `vnstock_data` ở đâu cả"*. Ngày **22/09** ĐO 14 thêm `import vnstock_data` vào `fetch_khoi_ngoai.py` và không ai sửa câu ấy; nó sống **2 ngày**, trần, ở file đứng đầu thứ tự ưu tiên. `tools/soat_loi_khai_cu.py` sinh ra đúng để bắt loại này, nhưng `PHU_DINH` không có *"không nhập"* — cả họ lời khai về QUAN HỆ giữa các mảnh mã (*X không nhập / gọi / dùng Y*) nằm ngoài tầm, dù đó là loại mục nhanh nhất khi mã đổi | tình cờ, khi viết tiêu chí ĐO 17: đọc lại lý do *"repo không nhập"* của BƯỚC 104 rồi `grep` | ⚠️ một phần | `PHU_DINH` thêm họ *không nhập · gọi · dùng · chạm* — đo trước cùng bộ lọc: **+5** dòng; bỏ *không đọc · có · còn*: **+23**, phần lớn là luật. Hai gác hai chiều ở `tests/test_soat_dinh_ky_co_tri_nho.py`, phát đầu dựng lại nguyên văn dòng `HANDOFF` ở `7a774a2`; đục **8/8 đỏ**. **Một máy ĐỌC theo nhịp soát, không phải cổng** — không chặn lúc commit — và chỉ bắt lời khai nằm trên MỘT dòng |

| 101 | **luật đã có sáu ngày, và bản nháp vẫn đi đường khác — vì luật nằm trong phần GIỚI HẠN của MỘT mục tiêu chí.** `docs/TIEU-CHI-DOC-TRUOC.md` ĐO 13 điều 5 (18/09): *"gộp hai phép nâng vào một lượt là đúng cái lỗi `--stride 1`"*. Bản nháp ĐO 17 (24/09) nâng **ba** gói trong một lượt; dòng *Đã tra trùng* đầu tiên của nó nêu ĐO 10 mà không nêu ĐO 13. Không vào commit nào — cùng lớp lỗi 56 và 81: luật đã ghi, và tôi đi đường khác vì việc **trông quen** | tự đọc lại tiêu chí ĐO 13 **trước khi ký**, lúc soạn câu hỏi cho sổ tay | ⚠️ một phần | luật được kéo ra khỏi mục ĐO 13 vào chỗ chung: `references/bay.md` mục *"Nâng hoặc cài một gói"*. Máy chỉ chặn cho ĐO 17: `tools/do17_nang_goi_vnstock.py::phan_xu` từ chối mọi chặng có freeze đổi khác đúng MỘT gói, đục **31/31 đỏ**. **Không dựng gác toàn repo** — không phép quét nào thấy một bản nháp chưa commit đang gộp gì |

| 102 | **máy đo so phán quyết CŨ/MỚI của một cửa nạp bản CŨ từ scratchpad — và bản ấy mang một hằng số tính từ CHỖ NÓ NẰM.** `cua_bash_an_toan.GOC` là `Path(__file__).resolve().parent.parent`: chép file sang scratchpad là đổi *gốc repo* thành scratchpad, nên với bản cũ mọi đường trong scratchpad đều thành *trong repo*. Lượt so trên nhật ký cửa (25/09) báo *"tha thêm 4"*, trong đó **1 là ảo** — một heredoc ghi ra scratchpad mà bản cũ THẬT không bao giờ chặn. Cùng họ lỗi 61: máy đo in ra một con số nghe hợp lý | tình cờ: bản vá sau chỉ đổi luật `pytest-qua-ong`, mà phép so vẫn báo `ghi-de-file-nguon` đổi phán quyết | ❌ | máy đo ép `cu.GOC = moi.GOC` rồi `assert`, đo lại: tha 6, chặn thêm 0. **Chưa có máy** — mọi phép so hai bản của một module có hằng số suy từ `__file__` đều mắc được; `docs/STATE.md` BƯỚC 122 |

| 103 | **tối ưu cho câu hỏi ĐO LƯỜNG thay cho mục tiêu của NGƯỜI DÙNG — kế hoạch đề xuất tắt đúng cái lõi người dùng dựng app để có.** Từ *"chiến lược không có alpha"* tôi suy ra *"bỏ máy tự giao dịch, đóng băng sổ lệnh ảo, bỏ nhãn MUA"*. Mục tiêu của dự án không đòi alpha dương: nó đòi một agent tự giao dịch ảo, tự học sau mỗi lệnh, và minh bạch để người học theo. Kết quả *"không có lợi thế"* là thông tin cho agent học, không phải lý do tắt agent | người dùng đọc kế hoạch (25/09): *"nó đã phá hỏng ý tưởng ban đầu của tôi"* | ❌ | không máy nào chặn được. Mục tiêu gốc ghi vào `docs/STATE.md` BƯỚC 122; luật làm việc: trước khi đề xuất bỏ một thành phần người dùng tự dựng, **hỏi mục đích của nó** |
