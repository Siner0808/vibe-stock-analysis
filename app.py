...
with tab_main:
    if not result:
        ...
        gauge.update_layout(height=280, template="plotly_dark", margin=dict(l=30, r=30, t=30, b=10), font={'color': '#ffffff'})
        ...
        bar_fig = go.Figure(go.Bar(x=agent_names, y=agent_scores, marker_color=colors, text=[f"{s:.0f}" for s in agent_scores], textposition="outside"))
        ...
        bar_fig.update_layout(template="plotly_dark", height=300, yaxis=dict(range=[0, 105]), margin=dict(l=20, r=20, t=20, b=20))
...
# ════════════════ TAB 2: DEBATE COUNCIL ════════════════
with tab_debate:
    if not result:
        st.info("👈 Hãy chạy phân tích để xem kết quả Debate Council.")
    else:
        ...
        if not debate:
            ...
            c1.metric("Điểm trước tranh luận", f"{pre:.1f}")
            c2.metric("Bull Score tổng", f"{debate['bull_score']:+.2f}")
            c3.metric("Bear Score tổng", f"{debate['bear_score']:+.2f}")
            c4.metric("Điều chỉnh cuối", f"{adj:+.1f}", delta=f"→ {post}", delta_color="normal")
            ...
            st.info(f"🛡️ **Độ tin cậy Debate Council:** {debate['confidence_level']} — Dựa trên mức độ đồng thuận/bất đồng giữa các phe tranh luận.")

# ════════════════ TAB 3: CHI TIẾT TỪNG AGENT ════════════════
with tab_detail:
    if not result:
    ...
    if not result:
        ...
        if not news_res or news_res.get("total_articles", 0) == 0:
            ...
            if src_names:
                ...
                src_fig = go.Figure(go.Bar(x=src_names, y=src_scores, marker_color=src_colors, text=[f"{s:+.1f}" for s in src_scores], textposition="outside"))
                src_fig.add_hline(y=0, line_color="#b0bec5")
                src_fig.update_layout(template="plotly_dark", height=250, yaxis_title="Điểm sentiment (-100 → +100)", margin=dict(l=20, r=20, t=10, b=20))
            ...
            if sector_sent:
                ...
                sec_fig = go.Figure(go.Bar(
                    x=sec_labels, y=sec_scores, marker_color=sec_colors,
                ...
                sec_fig.update_layout(template="plotly_dark", height=380, yaxis_title="Sentiment Score", xaxis_tickangle=-35, margin=dict(l=20, r=20, t=10, b=100))
...
with tab_chart:
    if not result:
        ...
        fig.update_layout(template="plotly_dark", height=500, xaxis_rangeslider_visible=False, margin=dict(l=20, r=20, t=20, b=20), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
        st.plotly_chart(fig, use_container_width=True)

        vol_colors = ['#00e676' if df['close'].iloc[i] >= df['open'].iloc[i] else '#ef5350' for i in range(len(df))]
        ...
        st.warning("Không tải được dữ liệu OHLCV. Vui lòng kiểm tra mã cổ phiếu.")

# ════════════════ TAB 6: SƠ ĐỒ PIPELINE ════════════════
with tab_diagram:
